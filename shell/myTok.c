...
//only execute this code if there are tokens left in the array. 
	...
	//we must walk the tokens left to right until <, >, or | are found.
	char** tokIter = parsedToks;
	
	//we need to find out the first block commands.
	while(*tokIter != 0){//only one of these will be true at any time.
		if(**tokIter == '<'){
			pipeType = 1;
			break;
		}else
	...
	//based on the type of pipe if any we need to take action appropriately.
	if(pipeType == 1){//if pipe was <
		int pid;
		int *pipeFds;
